...
class Requirement:
    def __init__(self, varPath: str, operator: str, value: str, typeName: str) -> None:
        ...
        (variable, methodCall, unused) = Feature.parseFeature([(varPath if self.getInfoFromMethod else varPath.replace("]", " ? ?]"))]) # use existing function to find out if requirement depends on value of a method
